...
export const Item = () => (data.map(
  ({imageUrl, id, name, city, country}) => (
    // eslint-disable-next-line react/jsx-key
   <div key={id} className="guests-container__item">
     <img className="guests-container__image-hotel guests-img" src={imageUrl}></img>
     <div className="places">
       <p className="guests-container__name">{name}</p>
       <p className="guests-container__place">{city}, {country}</p>
     </div>
   </div>
  )
)
